...
export class MyApp {
  ...
  setupPush(){
    ...
    if(Number(this.device.version) <=8.0){
    ...
    try{
    this.push.deleteChannel('tafelOutlet').then(() => console.log('Channel deleted'));
    }
    catch(error){
   ...
   pushObject.on('notification').subscribe((notification: any) => {
      ...
      let modal = this.modalCtrl.create(ContactPage,notification);
      modal.onDidDismiss((data) => {
     
      });
      modal.present();
      this.nav.push(ContactPage,{message:notification.message});
