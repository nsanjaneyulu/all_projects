...
public class SearchExpenses extends FarmPage {
	...
	protected void buildPageComponents() {
		...
		Form form = new Form("expensesearch");
		searchFields.add(buildExpenseType());		
...
private DataView<FarmObject> buildDataView(){
		...
		this.dataView = new DataView<FarmObject>("rows",new DynamicDataProviderWrapper<SearchExpenses>(this), paginationSize) {
			protected void populateItem(final Item<FarmObject> item) {
				IModel<FarmObject> expenseModel = (IModel<FarmObject>)item.getModel();
				final Expenses expense = (Expenses)expenseModel.getObject();
				Link editLink=new Link<FarmObject>("expense_edit", expenseModel) {
					public void onClick() {
				...
				editLink.add(new Label("rcptNumberlbl",
								new PropertyModel<String>(expense,
										"receiptNumber")));
	...
	private FormComponent<ExpenseType> buildExpenseType() {
		CriteionAwareDropDownChoice<ExpenseType> expenseType = new CriteionAwareDropDownChoice<ExpenseType>(
				"expenseType", new PropertyModel<ExpenseType>(this,"expenseType"),
	...
	private AjaxButton buildSearch(List<FormComponent<?>> searchFields) {
		...
		AjaxButton search = new AjaxButton("search") {
			...
			protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
				...
				String queryString = "select distinct l from Expenses l ";
				String countQueryString = "select count(distinct l) from Expenses l ";
