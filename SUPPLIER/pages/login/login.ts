...
export class LoginPage {
	...
	@ViewChild('login') loginInput: any;
	constructor(public navCtrl: NavController, public navParams: NavParams, private utilityProvider: UtilityProvider, private user: UserDataProvider, private alertCtrl: AlertController, private storage: Storage, public plt: Platform, private loadingCtrl: LoadingController, private uniqueDeviceID: UniqueDeviceID, private endPoint: endPointService) {
		this.getPlatform();
	...
	login() {
		...
		if (!phoneNumber) {
			...
			this.presentAlert('Login Failed1!', 'Please enter valid phone number1');
			this.phoneNumber = "";
			return false;
		}
		phoneNumber = phoneNumber.substring(1);
		if (phoneNumber && (phoneNumber) || phoneNumber.length < 10) {
			this.presentAlert('Login Failed2!', 'Please enter valid phone number2');
			phoneNumber = "";
			return false;
		} else {
			this.storage.get("DEVICE_REG_ID").then((val) => {
	...
	doLoginWithDevice() {
		...
		this.user.getSupplierAccount(this.phoneNumber, this.deviceRegisteredId, this.deviceType).subscribe(data => {
			...
			if (data.id && (data.type == "SUPPLIER" || data.type == "MALL_OWNER")) {
				...
				this.storage.set('EMAIL_ID', data.email)
	...
	createOTP() {
		...
		//this.user.otpValidator(this.platform,phoneNumber,this.deviceId);
		this.user.otpValidator(this.platform, phoneNumber, this.deviceId).subscribe(data => {
			...
			if (parseInt(res) == 1) {
				...
				//supplier is already exists with other supplier
				let alert = this.alertCtrl.create({
					title: "Supplier is already other device with same phone number.",
					buttons: ['Ok']
				});
				alert.present();
			} else {
				//no supplier created for this phone number
				let alert = this.alertCtrl.create({
					title: "No supplier is created on this phone number.",
					buttons: ['Ok']
				});
				alert.present();
			}
