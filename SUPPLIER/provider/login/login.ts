...
export class UserDataProvider {
	...
	otpValidator(platform, phoneNumber, deviceId){
		let url = admin_service_endpoint + '/otpValidator';
		let deviceTypee=''
		if(platform=='browser'){
			deviceTypee='android';
		}
		let body = {
			"deviceType":deviceTypee?deviceTypee:platform,
			"deviceId": deviceId ? deviceId : '1111-2222-5555',
			"phoneNumber": phoneNumber,
			"userType": "SUPPLIER"
		}
	...
	checkOTP(otp, platform, phoneNumber, deviceId): Observable < any > {
		console.log("manager otp",otp);
		let url = admin_service_endpoint + '/checkotpForSupplier';
		console.log("url",url)
		let body = {
			"otp":otp,
			"deviceType": platform,
