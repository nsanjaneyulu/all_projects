...
import {Order_Service_Url, supplier_item_refund,admin_service_endpoint, supplier_access_url, TAFEL_API_KEY,BomServiceEndPoint} from '../../common/endPointService';
/*
...
export class OrdersProvider {
	...
	getOrderCount(urlVal) {
		let url = BomServiceEndPoint + urlVal;
		let localData = this.http.get(url, {
			headers: this.headers
		});
		return localData;
	...
	getAndroidVersion(url){
		let keyurl = "";
		keyurl = url;
		console.log("keyurl", keyurl);
		//this.headers = this.utilityProvider.getHeadersForService();
		console.log(this.headers);
		var reurnVal = this.http.get(keyurl, {headers: this.headers});
		console.log(reurnVal);
		return reurnVal;
	  }
